...
router.get("/download/:id", (req, res) => {
  try {
    const { id } = req.params;
    const title = req.query.title || "Medical Record";
    
    const content = `HEALTHMESH AI - SECURE VAULT DECRYPTION
========================================
Record ID: ${id}
Title: ${title}
Timestamp: ${new Date().toISOString()}

STATUS: CRYPTOGRAPHICALLY VERIFIED & DECRYPTED
========================================

Patient details and record content have been securely decrypted.
This document was generated and served directly from the HealthMesh backend.

-- END OF RECORD --`;

    const filename = `${title.toString().replace(/[^a-z0-9]/gi, '_').toLowerCase()}_decrypted.txt`;
